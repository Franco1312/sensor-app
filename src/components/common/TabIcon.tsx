/**
 * TabIcon - Custom SVG icons for bottom tab navigation
 * Follows the design system with theme colors
 */
...
interface TabIconProps {
  name: 'home' | 'chart' | 'news' | 'settings';
  focused: boolean;
  size?: number;
}

export const TabIcon: React.FC<TabIconProps> = ({ name, focused, size = 24 }) => {
  const { theme } = useTheme();
  const color = focused ? theme.colors.primary : theme.colors.textSecondary;

  // Clean, minimal icon paths following Material Design style
