/**
 * InputIcon - Icon component for input fields
 */
...
interface InputIconProps {
  name: 'person' | 'lock' | 'visibility' | 'visibility_off';
  size?: number;
}
...
export const InputIcon: React.FC<InputIconProps> = ({ name, size = 20 }) => {
  const { theme } = useTheme();
  const iconColor = theme.colors.textSecondary;

  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path d={iconPaths[name]} fill={iconColor} />
    </Svg>
  );
};
